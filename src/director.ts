...
import { setGlassesLensOpacity, GLASS_LENS_OPACITY } from './figure.ts'
...
export function updateViewCam(dt: number, elapsed: number) {
  ...
  if (scrollP > 0.02 && camState !== 'scroll') {
    ...
    setGlassesLensOpacity(GLASS_LENS_OPACITY) // dark ray-bans again
  ...
  if (camState === 'scroll') {
    ...
    setGlassesLensOpacity(GLASS_LENS_OPACITY - 0.9 * smooth01((t2 - 0.78) / 0.17))
